...
class LFUCache:
    ...
    def get_freq_obj_update_mru(self,key):

        if self.key_node_map[key].freq not in self.freq_dll_map:
            self.freq_dll_map[self.key_node_map[key].freq] = DoublyLinkedList()
        obj = self.freq_dll_map[self.key_node_map[key].freq]
    ...
    def update_node_pos(self,key):
        ...
        self.get_freq_obj_update_mru(key)
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if len(self.key_node_map) < self.capacity:
            ...
            if key in self.key_node_map:
                ...
                self.min_freq = 1
                self.get_minfreq_obj_update_mru(key)
        else:
